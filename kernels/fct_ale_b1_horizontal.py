...
def generate_code(tuning_parameters):
    ...
    upper_bound_branch_rewrite = \
        "int temp_00 = nLevels[(elementsPerEdge[(blockIdx.x * 2)]) - 1];\n" \
        "int temp_01 = nLevels[levelBound - 1];\n" \
        "levelBound = (((temp_00 * (temp_00 > temp_01)) + (temp_01 * (temp_01 >= temp_00))) * (levelBound > 0)) + ((temp_00 * (temp_00 > 0)) * (levelBound <= 0));\n"
    compute_block = \
...
if __name__ == "__main__":
    ...
    if command_line.store:
        try:
            with open("fct_ale_b1_horizontal_{}_{}_{}_{}_{}.json".format(command_line.nodes, command_line.elements, command_line.edges, command_line.max_levels, command_line.real_type), "x") as fp:
